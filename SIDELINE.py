...
def GRN_background(geneList, numTopGenes, normCD, cudaUse, outDir, timeDelay, randomGenes):
    tcdfInput = "./TCDF_Output/inputTemp.csv"
    tcdfOutput= "./TCDF_Output/outputTemp.csv"
    allInteractions = pd.DataFrame(columns = geneList)
    for j in range(len(geneList)):
...
def calSigLvl(bg_df, gene): 
    bgAppCts = bg_df['genePair'].value_counts().sort_index()
    ...
    try: 
        ...
        sig_lvl = bg_df.loc[:, 'genePair'].mean()
    return sig_lvl
...
def BACKGROUND(outDir, geneList, permutations, rawCase, rawCtrl, cellNumber, zeroThresh, topGenes, cudaUse, timeDelay): 
    ...
    caseMctrl = case.subtract(ctrl)
    caseMctrl = normalize(caseMctrl, zeroThresh)
    cols = list(caseMctrl.columns)
    randomGenes = random.sample(cols, topGenes-1)
    ...
    for k in range(permutations):
        ...
        # 4. Detect gene interactions
        interactions = GRN_background(geneList, topGenes, caseMctrl, cudaUse, outDir, timeDelay, randomGenes)
...
def GRAPH(outDir, geneList, permutations, timeDelay):
    locCheck = outDir + '/Background'
    if os.path.isdir(outDir) == True: 
        os.chdir(outDir)
        os.mkdir('./GRN_Visualization')
        os.chdir('./GRN_Visualization')
    elif os.path.isdir(locCheck) == True:
        os.mkdir('../GRN_Visualization')
        os.chdir('../GRN_Visualization')
    if timeDelay ==0: 
    ...
    for g in geneList: 
        ...
        background.columns = ['Unnamed: 0', 'genePair']
